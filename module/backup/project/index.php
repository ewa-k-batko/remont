...
if ($this instanceof Manager_Controller) {
    ...
    $link = new Model_Link_Container();
    $link->setId(1)->setUrl($url . 'parter')->setTitle('Rzut parteru')->setClass('parter');
    $nav->append($link);

    $link = new Model_Link_Container();
    $link->setId(2)->setUrl($url . 'poddasze')->setTitle('Rzut poddasza')->setClass('poddasze');
    $nav->append($link);
   ...
   /* $link = new Model_Link_Container();
    ...
    $link = new Model_Link_Container();
    $link->setId(5)->setUrl($url . 'front-polnoc-luk' )->setTitle('Rzut frontu północnego')->setClass('front-polnoc-luk');
    $nav->append($link);
    
    $link = new Model_Link_Container();
    $link->setId(6)->setUrl($url . 'front-poludnie-luk' )->setTitle('Rzut frontu południowego')->setClass('front-poludnie-luk');
    $nav->append($link);
    
    $link = new Model_Link_Container();
    $link->setId(7)->setUrl($url . 'przekroj-wschod' )->setTitle('Rzut przekroju wschodniego')->setClass('przekroj-wschod');
    $nav->append($link);
    
     $link = new Model_Link_Container();
    $link->setId(8)->setUrl($url . 'przekroj-zachod')->setTitle('Rzut przekroju zachodniego')->setClass('przekroj-zachod');
    $nav->append($link);
